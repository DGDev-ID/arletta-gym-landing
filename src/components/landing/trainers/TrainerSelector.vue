...
<template>
  <Dialog
    ...
    :pt="{
      root: {
        class:
          'w-full max-w-[820px] mx-4 rounded-[20px] overflow-hidden border border-white/[0.08] shadow-[0_0_0_1px_rgba(230,33,41,0.15),0_40px_80px_rgba(0,0,0,0.7),0_0_60px_rgba(230,33,41,0.06)] bg-gradient-to-br from-[#1a1a1f] via-[#111114] to-[#0e0e11] flex flex-col max-h-[90vh]',
      },
  ...
  >
    <template #header>
      <div class="flex-1">
        ...
        <p class="text-xs text-white/40 m-0">
          Temukan trainer yang cocok dengan target latihan Anda
        </p>
      </div>
      ...
      >
        ...
        <span class="text-(--primary) font-bold ml-0.5"
          >Rp {{ (tempPackage.price / 1000).toFixed(0) }}k</span
        >
      </div>
    ...
    <div class="grid grid-cols-2 gap-3 p-5 max-sm:grid-cols-1">
      <div
        ...
        :class="{
          'border-(--primary)! bg-[rgba(230,33,41,0.06)]! -translate-y-0.5':
      ...
      >
        <div
          ...
          :class="{
            'bg-(--primary)! border-(--primary)! shadow-[0_0_10px_rgba(230,33,41,0.5)]':
              selectedTrainer?.id === t.id,
          }"
        >
          <i
            class="pi pi-check text-[10px] transition-all duration-200"
            :class="selectedTrainer?.id === t.id ? 'text-white' : 'text-transparent'"
          ></i>
        </div>
        ...
        <div class="relative w-fit">
          ...
          />
          <div
            class="absolute -inset-1 rounded-full border-2 transition-colors duration-300"
            :class="
              selectedTrainer?.id === t.id ? 'border-[rgba(230,33,41,0.35)]' : 'border-transparent'
            "
          ></div>
          <div
            class="absolute -bottom-1 -right-2 flex items-center gap-0.5 px-1.5 py-0.5 bg-black/90 border border-white/10 rounded-full text-[10px] font-bold text-amber-400"
          >
        ...
        <div class="flex flex-col gap-1.5 flex-1">
          ...
          <p class="text-[0.7rem] font-medium uppercase tracking-[0.06em] text-(--primary) m-0">
            {{ t.role }}
          </p>
          ...
          <div class="flex flex-wrap gap-1 mt-0.5">
            <span
              v-for="spec in t.specializations.slice(0, 3)"
              :key="spec"
            ...
            >
            <span
              v-if="t.specializations.length > 3"
              class="px-1.5 py-0.5 text-[0.65rem] text-white/30 italic"
              >+{{ t.specializations.length - 3 }} more</span
            >
          </div>
        ...
        <div
          class="pt-2 border-t transition-colors duration-300"
          :class="
            selectedTrainer?.id === t.id ? 'border-[rgba(230,33,41,0.2)]' : 'border-white/[0.06]'
          "
        >
          <span
            class="text-[0.72rem] font-semibold transition-colors duration-300"
            :class="selectedTrainer?.id === t.id ? 'text-(--primary)' : 'text-white/35'"
          >
