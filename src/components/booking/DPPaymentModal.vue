...
<template>
  <Dialog
    ...
    :pt="{
      root: { class: 'rounded-[20px] overflow-hidden border border-white/[0.08] shadow-[0_0_0_1px_rgba(230,33,41,0.15),0_40px_80px_rgba(0,0,0,0.7),0_0_60px_rgba(230,33,41,0.06)] bg-gradient-to-br from-[#1a1a1f] via-[#111114] to-[#0e0e11] flex flex-col max-h-[90vh]' },
      mask: { class: 'backdrop-blur-md !bg-black/75' },
      header: { class: '!flex items-center gap-3 !px-7 !pt-5 !pb-4 border-b border-white/[0.07] bg-black/30 flex-shrink-0 !rounded-none' },
      content: { class: '!p-0 bg-transparent overflow-y-auto flex-1' },
      footer: { class: '!flex justify-end gap-3 !px-7 !pt-1.5 !pb-[18px] border-t border-white/[0.07] bg-black/30 flex-shrink-0 !rounded-none !m-0' },
    }"
  >
    <template #header>
      <div class="flex-1">
        ...
        <p class="text-xs text-white/40 m-0">Pilih cara pembayaran yang sesuai dengan kenyamanan Anda</p>
      </div>

      <div v-if="pkg" class="flex items-center gap-1.5 mr-2 px-3 py-1.5 bg-[rgba(230,33,41,0.12)] border border-[rgba(230,33,41,0.3)] rounded-full text-xs text-white/80 whitespace-nowrap flex-shrink-0">
    ...
    <div v-if="pkg" class="flex flex-col gap-4 p-7">
      ...
      <div class="flex flex-col gap-3">
        ...
        <label
          ...
          :class="paymentType === 'full' ? 'bg-(--primary)/10 border-(--primary)/40' : 'bg-white/5 border-white/10 hover:bg-white/8'"
        >
        ...
        <label
          ...
          :class="paymentType === 'dp' ? 'bg-(--primary)/10 border-(--primary)/40' : 'bg-white/5 border-white/10 hover:bg-white/8'"
        >
          ...
          <div class="flex-1">
            ...
            <div class="text-xs text-(--text-muted) mb-3">Bayar sebagian di awal, sisanya sebelum sesi dimulai</div>
            <div v-if="paymentType === 'dp'" class="flex gap-2 mt-2">
              <button
                class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all"
                :class="dpPercentage === 30 ? 'bg-(--primary) text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'"
                @click.prevent="dpPercentage = 30"
              >30%</button>
              <button
                class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all"
                :class="dpPercentage === 50 ? 'bg-(--primary) text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'"
      ...
      <div v-if="paymentType === 'dp'" class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex flex-col gap-2">
        <div class="flex justify-between text-sm">
